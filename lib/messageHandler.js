...
export async function msg(client, m) {
    ...
    m.sender = m.key.fromMe ? client.decodeJid(client.user.id) : (m.key.participant || m.chat)

    let admins = await client.getAdmins(m.chat)
    ...
    m.reply = async (text, options = {}) => {
        if (typeof text === "string") {
            return await client.sendMessage(m.chat, { text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options })
        } else if (typeof text === "object") {
            return client.sendMessage(m.chat, { ...text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options })
        }
    }    
